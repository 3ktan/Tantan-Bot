...
class Ship:
    ...
    def from_file(cls, name, file):
        data = json.load(file)
        new_ship = cls(data['all_forms'], data['quests'])
        return new_ship
    ...
    def embed_form(self):
        ...
        embed.add_field(
            ...
            value="ID: "+self.all_forms["id"] + "\nClass: "+self.all_forms["class"]
        )
        return embed
...
class Kancolle:
    def __init__(self, bot, **kwargs):
        ...
        #     for i in file:
        #         i = i[:-1]
        #         with open("data/kancolle/ship/" + i +".json","r", encoding='utf-8') as file:
        ...
        for i in os.listdir(f"data/kancolle/ship"):
            with open(f"data/kancolle/ship/{i}", encoding='utf-8') as file:
                new_ship = Ship.from_file(i[:-5], file)
                self.ship_library.append(new_ship)
    ...
    async def ship(self, ctx, *, name: str):
        ...
        if len(result) > 1:
            await ctx.send("Do you mean:\n```\n{}\nc: cancel\n```".format(
                '\n'.join([str(index + 1) + ": " + c.all_forms for index, c in enumerate(result)])))
